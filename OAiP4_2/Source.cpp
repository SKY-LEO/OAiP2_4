...
bool rewriteToPolish(char*& input_string, char*& output_string, Input_Stack*& begin, int size_input_string)
{
	...
	Number_Of_Symbols* arr = new Number_Of_Symbols[size_input_string];//äëÿ ïîäñ÷åòà áàëàíñà îïåðàíäîâ è îïåðàòîðîâ
	for (int i = 0; i < size_input_string; i++)			//íóëåâàÿ ÿ÷åéêà äëÿ ïîäñ÷åòà âñåõ, äðóãèå - òîëüêî äëÿ âíóòðåííèõ ñêîáîê 
	{
		arr[i].letters = 0;
		arr[i].operators = 0;
	}
	for (int i = 0; input_string[i] != '\0' && !is_error; i++)
	{
		...
		if (symbol >= 'a' && symbol <= 'z')
		{
			...
			if (is_open_bracket)
			{
				for (int k = 1; k < count_of_open_brackets && k < size_input_string; k++)//íóëåâàÿ ÿ÷åéêà èñïîëüçóåòñÿ 
																			//äëÿ ïîäñ÷åòà âñåõ îïåðàòîðîâ è îïåðàíäîâ
				{
					arr[k].letters += 1;
				}
			}
		...
		else if (symbol == ')')
		{
			...
			if (!flag)//åñëè íå íàøëàñü îòêðûâàþùàÿ ñêîáêà
			{
			...
			{//åñëè áàëàíñ âíóòðè ñêîáîê íàðóøåí
	...
	else if (checkLetterAndOperatorBalance(arr[0].letters, arr[0].operators))//åñëè íå ðàâíî îáùåå ÷èñëî îïåðàíäîâ è îïåðàòîðîâ
	{
	...
	for (int i = 0; output_string[i] != '\0' && !is_error; i++)
	{
		...
		else
		{
			...
			case '^':
				...
				else if (var2 < 0. && (int)var1 != var1)//îòðèöàòåëüíîå ÷èñëî ìîæåò áûòü âîçâåäåíî òîëüêî â öêëóþ ñòåïåíü
				{
